# # What to do
# 2. Image augmentation: cropping, roatation, random erasing. Important! Also apply this on the mask etc not just the main image
# 3. Visualize Outputs on training - maybe every 10th layer or something?
# 4. Read up on architecture
# 5. Fix slider
...
#         class_labels = {0: "normal", 1: "aneurysm",}

#         images = [
#             wandb.Image(
#                 x[0].sum(dim=i).numpy(),
#                   masks={
#                       "prediction" : {"mask_data": prediction[0].sum(dim=i).numpy() > 0, "class_labels": class_labels},
#                       "ground truth" : {"mask_data": y[0].sum(dim=i).numpy() > 0, "class_labels": class_labels}
#                  })
#             for i in range(3)]

#         wandb.log({f'{level}_sample': images})
    ...
    def prepare_data(self):
        ...
        #resample = tio.Resample(0.27)
        #preprocessing = tio.Compose([rescale, resample])
        ...
        self.train_dataset = self.get_dataset(0, 60, train_transform)
        self.val_dataset = self.get_dataset(61, 95, preprocessing)
        self.test_datset = self.get_dataset(95, 109, preprocessing)
